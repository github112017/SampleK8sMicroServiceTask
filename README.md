...
The services use the Spring framework and two Spring profiles are provided for each of the services and the integration tests allowing different configuration in different environments. Profile `local` is used by default and  `prod` is used when running inside Kubernetes. The configuration includes HS256 signing keys (one shared between **publicservice** and **privateservice** and the other shared between **publicservice** and the integration tests).
...
#### 4. Expose Public-Service via Ingess
...
This will follow the log messages as they are emitted; wait until you see the "Started Application in XXX.XXX seconds" message.
...
```sh
spring_profiles_active=prod request_host=localhost/publicservice/v1 ./gradlew tests:integration:integration
```
...
1. I added more logging than needed for observability into the services
   ...
   would normally be downgraded to debug or trace levels (or removed completely) but are handy for debugging during development and testing.
2. The HS256 encryption algorithm is a symmetric algorithm meaning
...
3. Currently the HS256 signing keys are stored in the services and integration test's application properties files. This is a bad idea because they are stored in plaintext. A best practice is to not store production secrets in source code and instead store them directly in Kubernetes as [Kubernetes Secrets](https://kubernetes.io/docs/concepts/configuration/secret/) where they can be exposed only to the services that require them. However, I left them in the application properties for this assignment as the HS256 symmetric key must be also known to the integration tests (using an algorithm using a private/public key pair would resolve this).
4. The **privateservice** can only be called from **publicservice** as requested, this was implemented using a combination of schemes:
...
5. I quite liked this assignment. Despite having experience with Java
